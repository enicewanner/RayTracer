...
int main(int, char**)
{
	...
	Scene scene({ 1, 0.1, 0.1 }, {1, 1, 1});
	//Camera camera({ 0, 0, 2 }, { 0, 0, 0 }, { 0, 1, 0 }, 60.0f, width / (float)height);
...
{
	//light
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 5, 5, 3 }, 1.5f, std::make_unique<Emissive>(color3{ 40, 40, 40 })));

	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 0, 0, -1 }, 0.5f, std::make_unique<Lambertian>(color3{ 0.1, 1, 0.1 })));
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 1.3, 0, -1 }, 0.5f, std::make_unique<Metal>(color3{ 1, 1, 1 }, 0.3f)));
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ -1.3, 0, -1 }, 0.5f, std::make_unique<Dialectric>(color3{ 1, 1, 1 }, 1.4f)));

	scene.AddObject(std::make_unique<Plane>(glm::vec3{ 0, -1.5, 0 }, glm::vec3{ 0, 1, 0 }, std::make_unique<Lambertian>(color3{ 0.2f, 0.2f, 0.2f })));
}

void InitScene02(Scene& scene)
{
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 0, 1, 0 }, 1.0f, std::make_unique<Dialectric>(color3{ 1.0f, 1.0f, 1.0f }, 1.5f)));
		...
		for (int z = -11; z < 11; z++)
		{
			...
			if ((center - glm::vec3{ 4, 0.2, 0 }).length() > 0.9f)
			{
				...
				else // glass
				{
					...
					material = std::make_unique<Dialectric>(albedo, 1.5f);
				}
